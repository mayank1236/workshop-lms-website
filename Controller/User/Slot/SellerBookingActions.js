...
var acceptNewBooking = async (req, res) => {
  ...
  await sellerBookings
    .findOne({ _id: mongoose.Types.ObjectId(id) })
    .then(async (data) => {
      var payment_status = await userServiceCart
        .findOne({
      ...
      var userBookedSlotData = await userBookedSlot
        .findOneAndUpdate(
          {
      ...
      return sellerBookings
        ...
        .then(async (docs) => {
          ...
          var serviceCartData = await userServiceCart
            .findOneAndUpdate(
              {
          ...
          var userSub = await Usersubcription.find({
            userid: mongoose.Types.ObjectId(docs.seller_id),
            status: true,
          }).exec();
          ...
          if (req.user.currency != payment_status.currency) {
            let convert = await Curvalue.find({
              from: payment_status.currency,
              to: req.user.currency,
            }).exec();
          ...
          let totalEarningData = await sellerTotalEarning
            .findOne({ seller_id: docs.seller_id })
            .exec();
  ...
  return sellerBookings.findOneAndUpdate(
    ...
    async (err, docs) => {
      if (!err) {
        ...
        var inServiceCart = await userServiceCart
          .findOne({
            user_booking_id: docs.user_booking_id,
            status: true,
          })
        ...
        var checkedOut = await userServiceCart
          .findOne({
            user_booking_id: docs.user_booking_id,
            status: false,
          })
        ...
        if (inServiceCart != null && checkedOut == null) {
          var bookingStatus = userServiceCart
            .findOneAndUpdate(
              {
                user_booking_id: docs.user_booking_id,
                // status: true
    ...
    .aggregate([
      {
        $match: {
          $and: [
            {
              $or: [{ rejected_by: "seller" }, { rejected_by: "buyer" }],
            },
            { new_booking: false, booking_accept: false },
          ],
        },
      },
...
var completeBooking = async (req, res) => {
  ...
  return userServiceCart
    ...
    .then(async (docs) => {
      let enableUserRefund = await sellerBookings
        .findOneAndUpdate(
          { user_booking_id: docs.user_booking_id },
          { completestatus: "complete" }
        )
