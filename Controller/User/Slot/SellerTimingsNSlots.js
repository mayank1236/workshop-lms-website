...
var sellerOff = require("../../../Models/Slot/sellerOffTiming");
    ...
    sellerTimings
      ...
      .then((result) => {
        ...
        if (result == null || result == "") {
          ...
          var timeRanges = getTimeRanges(
            ranges,
            st_InMin4m12AM,
            et_InMin4m12AM,
            interval,
            language
          ); // see below for utility functions
...
var viewShopServiceTimings = async (req, res) => {
  ...
  return sellerTimings
    .find({ shop_service_id: mongoose.Types.ObjectId(shop_service_id) })
    .then((data) => {
      if (data == null || data == "") {
        res.status(200).json({
          status: true,
          message:
            "This seller currently isn't providing any service in this category.",
          data: data,
        });
      } else {
        res.status(200).json({
...
var editTimingNSlots = async (req, res) => {
  ...
  return sellerTimings
    .findOneAndUpdate({ _id: mongoose.Types.ObjectId(id) }, req.body, {
      new: true,
    })
    .then(async (docs) => {
...
var deleteTimingNSlots = async (req, res) => {
  ...
  var deleteTiming = await sellerTimings.findOneAndDelete({
...
// Add Off Days
var createOffDays = async (req, res, next) => {
  const V = new Validator(req.body, {
    offDate: "required",
  });
  let matched = V.check().then((val) => val);
...
var viewShopOffServiceTimings = async (req, res) => {
  let shop_service_id = req.params.id; // shop_service_id in params
  return sellerOff
    .find({ shop_service_id: mongoose.Types.ObjectId(shop_service_id) })
    .then((data) => {
      if (data == null || data == "") {
        ...
        res.status(200).json({
          status: true,
          message: "Off Days for the seller",
          data: data,
        });
      }
    })
...
var deleteOffTiming = async (req, res) => {
  // delete with _id of seller_timings table
  var id = req.params.id;

  return sellerOff.findOneAndDelete(
    {
      _id: mongoose.Types.ObjectId(id),
    },
    (err, docs) => {
      if (!err) {
        res.status(200).json({
          status: true,
          message: "Off Days deleted successfully",
          data: null,
        });
      } else {
        res.status(500).json({
