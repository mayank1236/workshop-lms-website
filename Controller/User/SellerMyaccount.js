...
var serviceBookingStat = async (req, res) => {
  ...
  var findTotalBookings = await SellerBookings.find({
...
var wallet = async (req, res) => {
  ...
  /**----------------------------- Total earnings -----------------------------*/
  let commissionData = await SERVICE_SALE_EARNINGS.find({
    seller_id: mongoose.Types.ObjectId(id),
  }).exec();
  ...
  commissionData.forEach((element) => {
    totalEarnings =
      parseInt(totalEarnings) + parseInt(element.seller_commission);
  });
  /**--------------------------------------------------------------------------*/
  ...
  if (settledEarningData.length > 0) {
    settledEarningData.forEach((element) => {
      settledEarning =
        parseInt(settledEarning) + parseInt(element.seller_commission);
    });
  } else {
    settledEarning = 0;
  }
  ...
  if (pendingSettlementData.length > 0) {
    pendingSettlementData.forEach((element) => {
      pendingSettlement =
        parseInt(pendingSettlement) + parseInt(element.seller_commission);
    });
  } else {
    pendingSettlement = 0;
  }
  ...
  if (claimableEarningData.length > 0) {
    claimableEarningData.forEach((element) => {
      claimableEarning =
        parseInt(claimableEarning) + parseInt(element.seller_commission);
    });
  } else {
    claimableEarning = 0;
  }
