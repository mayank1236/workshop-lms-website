  ...
  return User.aggregate([
...

    {
      $lookup: {
        ...
        foreignField: "user_id",
        pipeline: [
          { $match: { $expr: { $and: [{ $eq: ["$refund_claim", true] }] } } },
...
var bookingNUserStat = async (req, res) => {
  ...
  if (totalBookingsCount > 0) {
    var findAcceptedBookings = await sellerBookings
      .find({
        new_booking: false,
        booking_accept: true,
      })
      .exec();
    var findRejectedBookings = await sellerBookings
      .find({
        new_booking: false,
        booking_accept: false,
      })
      .exec();
    var findPendingBookings = await sellerBookings
      .find({
        new_booking: true,
        booking_accept: false,
      })
